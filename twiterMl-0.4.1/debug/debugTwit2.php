...
foreach($contenido["results"] as $twit){
	//if($twit["metadata"]["result_type"] == "popular"){
		echo "<br>".$twit["metadata"]["result_type"];
	//}
	//$twits->addTwit($twit);
	//$twits->contarTwits($twit["text"]);
	//$estats->addPalabra($twit["text"]);
}

$twits->listaTwits = $twits->twitsJson();

//var_dump($twits->listaTwits);
//var_dump($twits->listaPalabras);
